...
describe('Races', () => {
  ...
  it('should have a form input called raceOrder', () => {
    expect(element(by.css('input[formcontrolname=newRaceOrder]')).isPresent()).toBe(true);
  });
  ...
  it('should create new race on sumbmit', () => {
    ...
    element(by.css('input[formcontrolname=newName]')).sendKeys('Race 1');
    ...
    expect(element(by.id('submit')).isEnabled()).toBe(false);
    expect(element(by.css('input[formcontrolname=newRaceOrder]')).getAttribute('value')).toBe('2');
  ...
  it('should create a second new race on sumbmit', () => {
    ...
    element(by.css('input[formcontrolname=newName]')).sendKeys('Race 2');
    expect(element(by.id('submit')).isEnabled()).toBe(true);

    element(by.id('submit')).click();
    browser.wait(ExpectedConditions.visibilityOf(element(by.id('name-1'))), 5000) ;
    expect(element(by.id('submit')).isEnabled()).toBe(false);
    expect(element(by.css('input[formcontrolname=newRaceOrder]')).getAttribute('value')).toBe('3');
